...
class CMR2(object):
    ...
    def __init__(self, recall_mode, model_num, params, LSA_mat, pres_sheet, rec_sheet):
        ...
        self.M_CF_sem = np.identity(
            self.nelements, dtype=np.float32) # identity matrix is used to initialize the semantic matrix
...
def main():
    ...
    params = {
        ...
        'beta_enc':  0.341612498606514,           # rate of context drift during encoding
        'beta_rec':  0.9422648934928799           # rate of context drift during recall
        'beta_rec_post': 1,      # rate of context drift between lists
                                        # (i.e., post-recall)

        'gamma_fc': 0.16541698697170362,  # learning rate, feature-to-context
        'gamma_cf': 0.5733634902506061,  # learning rate, context-to-feature
        'scale_fc': 1 - 0.16541698697170362,
        'scale_cf': 1 - 0.5733634902506061,


        's_cf': 1.012142140864605,       # scales influence of semantic similarity
                                # on M_CF matrix
        ...
        'phi_s': 2.7362942742031033,      # primacy parameter
        'phi_d': 0.49618242724041345,      # primacy parameter


        'epsilon_s': 0.0,      # baseline activiation for stopping probability 
        'epsilon_d': 2.205181032342664,        # scale parameter for stopping probability 

        'k_intra':  5.857486202812575,        # scale parameter in luce choice rule during recall
        'rational': 0,
        'rational_cue': 0,       
        'weighted': 1,  
