...
#                 self.c_net[:nelements_temp] = ctemp_updated
    ...
    def present_list(self):
        ...
        for i in range(self.listlength):
            ...
            # give additional encoding strength if the item is semantically congruent with current context
            #net_cin = np.dot(self.M_FC_sem, self.f_net.T)
            #cos_similarity = scipy.spatial.distance.cosine(net_cin,self.c_old)
            #lr = np.exp(-self.params['lr']*cos_similarity)
             
                
            net_cin = np.dot(self.M_FC_sem, self.f_net.T)
            cos_similarity = 1-scipy.spatial.distance.cosine(net_cin,self.c_old)
            lr = self.params['lr']*cos_similarity+1
                       
            
            # Update M_FC
            M_FC_exp = np.dot(self.c_old, self.f_net)
            self.M_FC_tem += M_FC_exp * lr

  
            # Update M_CF
            M_CF_exp = np.dot(self.f_net.T, self.c_old.T)
            self.M_CF_tem += M_CF_exp * prim_vec[i] * lr
