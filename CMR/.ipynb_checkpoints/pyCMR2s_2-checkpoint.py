# This is modified from pyCMR2s.py but does not allow recalling just-recalled item
...
class CMR2(object):
    ...
    def retrieve_next(self, in_act):
        ...
        # all items in the list are possible to be recalled except for the just-recalled item
        support = [np.max([in_act[0,:nitems_in_session][i],0]) if self.torecall_list[0,:nitems_in_session][i]==1 and self.just_recalled!= i else 0.0 for i in range(len(in_act[0,:nitems_in_session]))] 
        ...
        for r in range(int(self.params['Num'])):
            ...
            bins = [bins[i] if self.torecall_list[0,:nitems_in_session][i]==1 and self.just_recalled!= i else 0.0 for i in range(len(in_act[0,:nitems_in_session]))] # only recall within-list items 
    ...
    def recall_session(self):
        ...
        continue_recall = 1
        self.just_recalled = -1 # nothing has been recalled at first
