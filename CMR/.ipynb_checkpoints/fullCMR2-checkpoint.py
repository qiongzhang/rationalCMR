...
class CMR2(object):
    ...
    def __init__(self, params, LSA_mat, data_mat):
        ...
        # MODIFIED - track M_CF for temporal context and semantic context separately
    ...
    def recall_session(self):
        ...
        # MODIFIED: track what are the items to recall for this particular list:
        ...
        #self.torecall = np.zeros([1, self.nelements], dtype=np.float32)
        ...
        while ((time_passed < rec_time_limit)
            ...
            c_similarity = np.dot(self.c_net.T, self.c_in_normed)
            ...
            if (winner_accum_idx is not None) and (
                ...
                if self.learn_while_retrieving:
                    ...
                    # MODIFIED: update separate M_CF_tem
                   # self.M_CF_tem += np.multiply(M_CF_exp, self.gamma_cf_retrieval)
        ...
        return recalled_items, RTs, times_since_start, self.M_CF[:,thislist_pres_indices], contexts_drift, contexts_IN
    ...
    def present_list(self):
        ...
        for i in range(self.listlength):
            ...
            # MODIFIED: update separate M_CF_tem
            # self.M_CF_tem += np.multiply(M_CF_exp, self.params['gamma_cf']) * prim_vec[i]
...
def run_CMR2_singleSubj(data_mat, LSA_mat, params):
    ...
    for i in range(len(this_CMR.pres_list_nos)):
        ...
        # recall session
        rec_items_i, RTs_list_i, times_from_start_i, contexts_study, contexts_drift, contexts_IN \
