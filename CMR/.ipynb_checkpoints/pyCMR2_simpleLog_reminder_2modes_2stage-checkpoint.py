...
class CMR2(object):
    ...
    def __init__(self, recall_mode, model_num, params, LSA_mat, pres_sheet, rec_sheet):
        ...
        # M_CF is used when considering rational drifting
    ...
    def retrieve_next(self, in_act):
        ...
        # [1] and [3] is used to collectively determin stopping probability 
        #if self.model_num < 3:
        ...
        #else:
        #    z = self.params['epsilon_s'] + self.params['epsilon_d']*np.sum(support_correct)
        #    p_stopping = 1/(1 + np.exp(-z))            
        ...
        if np.sum(support_correct) == 0: # terminate recall if there are no more within-list support values 
            ...
            if stopping: # if terminate 
                ...
                #if self.model_num%3 == 0:
                # luce choice rule with memory activation from remaining correct items, k controls noise level
                bins = np.exp(np.multiply(self.params['k_intra'],support_correct))          
                #elif self.model_num%3 == 1:
                #    bins = [np.exp(np.multiply(self.params['k_intra'],support_possible[i])) if self.torecall[0,:nitems_in_session][i]==1 else np.exp(np.multiply(self.params['k_extra'],support_possible[i])) for i in range(len(support_possible))]
                #elif self.model_num%3 == 2:    
                #    bins = [np.exp(np.multiply(self.params['k_intra'],support_possible[i])) if self.torecall[0,:nitems_in_session][i]==1 else self.params['k_extra']*np.exp(np.multiply(self.params['k_intra'],support_possible[i])) for i in range(len(support_possible))]
                
      
                #bins = [bins[i] if self.torecall[0,:nitems_in_session][i]!=-1 else 0.0 for i in range(len(in_act[0,:nitems_in_session]))] # only consider non-recalled items, including extralists
                ...
                out_statement = (winner_index, support_possible[winner_index])
    ...
    def retrieve_probability(self, in_act, recall_idx, stopping):
        ...
        if stopping: 
            ...
            #bins = [bins[i] if self.torecall[0,:nitems_in_session][i]!=-1 else 0.0 for i in range(len(in_act[0,:nitems_in_session]))] # only consider non-recalled items
    ...
    def recall_session(self):
        ...
        if self.params['cue_position'] == 0:        
            self.beta_in_play = self.params['beta_rec_bg'] 
        else: 
            self.beta_in_play = self.params['beta_rec_ed'] 
        ...
        if self.recall_mode == 0: # simulations 
            ...
            while continue_recall == 1:
                ...
                # MODIFIED: select optimal cue
                #if self.recall_method == 1:
                #    f_in_tem = np.dot(self.M_CF_tem, self.c_net)
                #    f_in_sem = np.dot(self.M_CF_sem, self.c_net)/self.params['s_cf']
                #    utility_tem = np.sum(np.dot(self.torecall, f_in_tem))
                #    utility_sem = np.sum(np.dot(self.torecall, f_in_sem))
                #    if utility_tem > utility_sem:
                #        f_in = f_in_tem * (1.0 + self.params['s_cf'])
                #    else:
                #        f_in = f_in_sem * (1.0 + self.params['s_cf']) 
                ...
                if winner_idx is not None:
                    ...
                    self.torecall_list[0][winner_idx] = 0
                    ...
                    contexts_drift = np.append(contexts_drift, self.c_old,axis=1)         
                else:
                    continue_recall = 0
            ...
            for i in range(recall_length):
                ...
                if recall_idx < len(self.all_session_items_sorted):
                    #f_in = np.dot(self.M_CF, self.c_net) # obtain item activations
                    ...
                    self.torecall_list[0][recall_idx] = 0
                    self.recalled[0][recall_idx] = 1
        ...
        return recalled_items, support_values, self.M_CF_tem[:,thislist_pres_indices], contexts_drift, contexts_IN
...
#                 self.c_net[:nelements_temp] = ctemp_updated
    ...
    def update_context_recall(self):
        ...
        # get input to the new context vector
        if self.params['cue_position'] == 0:
            net_cin = np.dot(self.M_FC_tem*self.params['gamma_fc_bg']+self.M_FC_sem*(1-self.params['gamma_fc_bg']), self.f_net.T)
        else:    
            net_cin = np.dot(self.M_FC_tem*self.params['gamma_fc_ed']+self.M_FC_sem*(1-self.params['gamma_fc_ed']), self.f_net.T)
        ...
        if self.params['cue_position'] == 0:        
            ctemp_updated = advance_context(
            cin_normed, net_c_temp, self.params['beta_rec_bg'])
        else:    
            ctemp_updated = advance_context(
            cin_normed, net_c_temp, self.params['beta_rec_ed'])
    ...
    def present_list(self):
        ...
        for i in range(self.listlength):
            ...
            # record cues
            if i==0:
                self.c_primacy = self.c_net.copy()
            ...
            # Update M_FC
            if np.argmax(np.random.multinomial(1,[0,1])):
                M_FC_exp = np.dot(self.c_old, self.f_net)
                #self.M_FC_tem += np.multiply(M_FC_exp, self.params['gamma_fc'])
        ...
        # MixturePrimacy    
        #self.c_cue = self.params['primacy'] * self.c_primacy + (1-self.params['primacy'])* self.c_net.copy() 
...
def run_CMR2_singleSubj(recall_mode, model_num, pres_sheet, rec_sheet, LSA_mat, params):
    ...
    for i in range(len(this_CMR.pres_list_nos)):
        ...
        # recall session
        rec_items_i, support_i, contexts_study, contexts_drift, contexts_IN \
            = this_CMR.recall_session()
        

        #plt.rcParams['figure.figsize'] = (15,3)
        #if i<5 and this_CMR.visualize:
            #plt.subplot(1,5,i+1)
        ...
        #plt.savefig('./Figs/plot0508policy.png')
        #plt.show()
        #plt.close('all')
...
def main():
    ...
    params = {
        ...
        'beta_enc':  0.32645788095960626,           # rate of context drift during encoding
        'beta_rec':  0.9396633230468256,           # rate of context drift during recall
        'beta_rec_post': 1,      # rate of context drift between lists
                                        # (i.e., post-recall)

        'gamma_fc': 0.17706827650611878,  # learning rate, feature-to-context
        'gamma_cf': 0.5986048837791892,  # learning rate, context-to-feature
        'scale_fc': 1 - 0.17706827650611878,
        'scale_cf': 1 - 0.5986048837791892,


        's_cf': 1.0695598102108046,       # scales influence of semantic similarity
                                # on M_CF matrix
        ...
        'phi_s': 2.452872419399586,      # primacy parameter
        'phi_d': 0.47140696798777765,      # primacy parameter


        'epsilon_s': 0.0,      # baseline activiation for stopping probability 
        'epsilon_d': 2.1917663226870188,        # scale parameter for stopping probability 

        'k_intra':  5.8099006828293405,        # scale parameter in luce choice rule during recall
    ...
    rec_path = '/mnt/bucket/people/qiongz/optimalmemory/pyCMR2/autolab_files/autolab_recs.txt'
    subjects_path = '/mnt/bucket/people/qiongz/optimalmemory/pyCMR2/autolab_files/autolab_subject_id.txt'
    ...
    resp, _,lkh = run_CMR2(
        ...
        params=params, subj_id_path=subjects_path, sep_files=False)
    print(lkh)
    print("End of time: " + str(time.time() - start_time))
    # save CMR2 results
    np.savetxt('./output/Z19_simulated.txt',
